"""
monitor.py — AI-powered self-healing monitor for Gamarr.
...
Integration in app.py:
    ...
    _monitor = monitor.GamarrMonitor(
        ...
        qb_reauth=lambda: qb_login(),
        clear_myrient_cache=_clear_myrient_cache,
        run_orphan_recovery=_run_orphan_recovery,
...
logger = logging.getLogger("gamarr.monitor")
...
SYSTEM_PROMPT = """You are an AI monitor for Gamarr, a self-hosted game and ROM download manager.
...
class GamarrMonitor:
    """AI-powered self-healing monitor for Gamarr."""
    ...
    def __init__(
        ...
        *,
        ...
        self._clear_myrient_cache = clear_myrient_cache
        self._run_orphan_recovery = run_orphan_recovery
    ...
    def _execute_action(self, action, params):
        try:
            dispatch = {
                "retry_job":             lambda: self._act_retry_job(params.get("job_id")),
                "clear_interrupted":     self._act_clear_interrupted,
                "reauth_qbittorrent":    self._act_reauth_qbittorrent,
                "refresh_myrient_cache": self._act_refresh_myrient_cache,
                "run_orphan_recovery":   self._act_run_orphan_recovery,
    ...
    def _act_refresh_myrient_cache(self):
        self._clear_myrient_cache()
        return "Myrient directory cache cleared"

    def _act_run_orphan_recovery(self):
        self._run_orphan_recovery()
